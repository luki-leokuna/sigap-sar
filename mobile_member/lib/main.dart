...
const _timsarOrigin = 'https://sigap-sar-production.up.railway.app';
...
class _TimsarWebShellState extends State<TimsarWebShell> {
  ...
  Timer? _compassTimer;
  double _progress = 0;
  String _cellStatus = 'Mengaktifkan Sistem Radar & Sensor SIGAP-SAR...';
  ...
  Future<void> _prepareAndLoad() async {
    ...
    if (!status.isGranted) {
      if (mounted) {
        setState(
          () => _cellStatus = 'Izin GPS & Sensor BTS diperlukan untuk Operasi SIGAP-SAR',
        );
      }
    } else if (mounted) {
      setState(() => _cellStatus = 'Mendeteksi Menara Seluler (BTS) & Serving Cell...');
    }
